...
const CompoundInterestCalculator = () => {
  ...
  const calculateCompoundInterest = () => {
    let n = 1; // Default to yearly compounding
    if (compoundingFrequency === "half-yearly") n = 2;
    if (compoundingFrequency === "quarterly") n = 4;

    const rate = annualRate / 100;
    const total = principal * Math.pow(1 + rate / n, n * timePeriod);
  ...
  }, [principal, annualRate, timePeriod, compoundingFrequency]);
      ...
      <div className="max-w-4xl w-full bg-white bg-opacity-10 shadow-lg rounded-lg p-6 flex flex-col lg:flex-row items-center space-x-8">
        <div className="w-full lg:w-1/2">
          ...
          <p className="text-gray-300 text-center mb-6">
            Calculate the Compound Interest and Total Amount based on your inputs.
          </p>
            ...
            <div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <input
                  ...
                  value={principal}
                  onChange={(e) => setPrincipal(Number(e.target.value))}
                  className="w-32 px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-yellow-400 text-black text-lg appearance-none"
                />
              </div>
            </div>
